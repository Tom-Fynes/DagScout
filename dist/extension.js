...
async function scanWorkspaceForPipelines(rootPath) {
  const nodes = [];
  const edges = [];
  const mapping = {};
  const dagPy = await (0, import_glob.glob)("**/*_dag.py", { cwd: rootPath, nodir: true, absolute: true });
  const pyFiles = await (0, import_glob.glob)("**/*.py", { cwd: rootPath, nodir: true, absolute: true });
  const toParsePy = new Set(dagPy);
  ...
  const sqlFiles = await (0, import_glob.glob)("**/*.sql", { cwd: rootPath, nodir: true, absolute: true });
  ...
  const ymlFiles = await (0, import_glob.glob)("**/*.yml", { cwd: rootPath, nodir: true, absolute: true });
...
function parseAirflowPython(content, file, nodes, edges, mapping) {
...
function parsePrefect(content, file, nodes, edges, mapping) {
  var _a;
  const lines = content.split(/\r?\n/);
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].trim().startsWith("@task")) {
      const next = (_a = lines[i + 1]) == null ? void 0 : _a.match(/def\s+(\w+)/);
      if (next) {
...
function parseDbtSql(content, file, nodes, edges, mapping) {
...
function parseYamlForDbt(content, file, nodes, edges, mapping) {
  try {
    const doc = load(content);
    if ((doc == null ? void 0 : doc.models) && Array.isArray(doc.models)) {
...
function parseGithubActions(content, file, nodes, edges, mapping) {
  try {
    const doc = load(content);
    if (!(doc == null ? void 0 : doc.jobs)) return;
...
function activate(context) {
  const disposable = vscode.commands.registerCommand("dagscout.openGraph", async () => {
    var _a;
    const root = (_a = vscode.workspace.workspaceFolders) == null ? void 0 : _a[0];
    if (!root) {
    ...
    const panel = vscode.window.createWebviewPanel(
      ...
      {
        enableScripts: true,
        localResourceRoots: [vscode.Uri.file(path2.join(context.extensionPath, "media"))]
      }
    );
...
function getWebviewContent(webview, mermaidSrc) {
  return `<!doctype html>
<html>
<head>
<meta charset="utf-8" />
